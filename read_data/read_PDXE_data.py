...
def read_PDXE_data(data_types=['rnaseq'],
                tissues=None, 
                projects=None,
                folder='../../data/PDXE/'):

    # If folder is None, set to default
    data_folder = folder or '../../data/PDXE/'

    if type(tissues) == str:
        tissues = [tissues]

    data_df = {'%s_%s'%('-'.join(tissues),d):read_one_PDXE_modality(tissues, projects, d, data_folder)\
            for d in data_types}
...
def read_one_PDXE_modality(tissues,
                    projects,
                    data_type,
                    folder):
    ...
    folders = np.array(['%s/%s/PDXE_%s_data.pkl'%(folder,
                                                data_type,
                                                data_type)
